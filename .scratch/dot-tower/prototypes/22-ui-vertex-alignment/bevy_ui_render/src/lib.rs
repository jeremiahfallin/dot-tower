...
// TICKET 22 PATCH -- field order changed, `_pad` added. See the prototype
// README. Upstream order put `color` at byte 20 and `border` at 56, so no
// vec4 attribute was ever 16-byte aligned and the 88-byte stride pushed the
// misalignment around on every vertex. The order below puts all three vec4s
// on 16-byte boundaries and rounds the stride to 96 so that holds for every
// vertex index. Field *order* is the only thing that changed -- the named
// struct literals at the push sites are order-independent, so they needed
// nothing but the new `_pad` field.
#[repr(C)]
...
struct UiVertex {
    ...
    /// offset 80
    pub point: [f32; 2],
    /// TICKET 22 PATCH -- rounds the stride from 88 to 96 so the 16-byte
    /// alignment above holds at every vertex index, not just vertex 0.
    /// `Pod` forbids implicit padding, so this has to be a real field.
    pub _pad: [f32; 2],
}
...
// TICKET 22 PATCH -----------------------------------------------------------
#[cfg(test)]
mod ticket_22_vertex_alignment {
    use super::UiVertex;
    use crate::pipeline::{UI_VERTEX_ATTRS, UI_VERTEX_STRIDE};
    use bevy_render::render_resource::VertexFormat;
    use core::mem::{offset_of, size_of};

    /// The CPU struct and the GPU vertex layout are two independent
    /// declarations of the same bytes, in two different files. If they drift,
    /// nothing fails to compile and nothing validates -- the GPU just reads the
    /// wrong fields, which is a quieter version of the bug being investigated.
    #[test]
    fn cpu_struct_matches_declared_gpu_layout() {
        let by_location = |loc: u32| {
            UI_VERTEX_ATTRS
                .iter()
                .find(|&&(_, _, l)| l == loc)
                .copied()
                .expect("every shader location is declared")
        };

        for (loc, name, cpu_offset, expected) in [
            (0, "position", offset_of!(UiVertex, position), VertexFormat::Float32x3),
            (1, "uv", offset_of!(UiVertex, uv), VertexFormat::Float32x2),
            (2, "color", offset_of!(UiVertex, color), VertexFormat::Float32x4),
            (3, "flags", offset_of!(UiVertex, flags), VertexFormat::Uint32),
            (4, "radius", offset_of!(UiVertex, radius), VertexFormat::Float32x4),
            (5, "border", offset_of!(UiVertex, border), VertexFormat::Float32x4),
            (6, "size", offset_of!(UiVertex, size), VertexFormat::Float32x2),
            (7, "point", offset_of!(UiVertex, point), VertexFormat::Float32x2),
        ] {
            let (format, gpu_offset, _) = by_location(loc);
            assert_eq!(format, expected, "{name}: format");
            assert_eq!(
                cpu_offset as u64, gpu_offset,
                "{name}: CPU struct offset does not match the vertex layout"
            );
        }
    }

    /// The point of the patch. Every attribute has to sit on a boundary that is
    /// a multiple of its own component width, and -- the part upstream misses --
    /// the stride has to preserve that for every vertex index, not just vertex
    /// zero. Upstream fails this at locations 2, 4 and 5.
    #[test]
    fn every_attribute_is_naturally_aligned_at_every_vertex_index() {
        assert_eq!(size_of::<UiVertex>() as u64, UI_VERTEX_STRIDE);
        assert_eq!(UI_VERTEX_STRIDE % 16, 0, "stride must not shift alignment");

        for (format, offset, loc) in UI_VERTEX_ATTRS {
            let align = format.size(); // 16 for vec4, 8 for vec2, 4 for u32
            let align = if align == 12 { 4 } else { align }; // vec3 packs at 4
            assert_eq!(
                offset % align,
                0,
                "location {loc} at byte {offset} is not aligned to {align}"
            );
        }
    }
}
...
// TICKET 22 PATCH -----------------------------------------------------------
/// The baseline this patch exists to contradict. Reproduces exactly what
/// `VertexBufferLayout::from_vertex_formats` does to the *stock* 0.19.1
/// attribute list (`offset += format.size()`, no padding) and records which
/// attributes come out misaligned, so the claim on ticket 22 is a measurement
/// rather than arithmetic done by hand.
#[cfg(test)]
mod ticket_22_stock_baseline {
    use bevy_render::render_resource::VertexFormat;

    /// The stock list, in `pipeline.rs` order, as of bevy_ui_render 0.19.1.
    const STOCK: [VertexFormat; 8] = [
        VertexFormat::Float32x3, // position
        VertexFormat::Float32x2, // uv
        VertexFormat::Float32x4, // color
        VertexFormat::Uint32,    // mode
        VertexFormat::Float32x4, // border radius
        VertexFormat::Float32x4, // border thickness
        VertexFormat::Float32x2, // border size
        VertexFormat::Float32x2, // position relative to the center
    ];

    #[test]
    fn stock_layout_misaligns_every_vec4() {
        let mut offset = 0u64;
        ...
        assert_eq!(stride, 88, "stock stride");
        assert_ne!(stride % 16, 0, "stock stride shifts alignment per vertex");

        let misaligned: Vec<_> = offsets
            .iter()
            .enumerate()
            .filter(|(_, (format, off))| *format == VertexFormat::Float32x4 && off % 16 != 0)
            .map(|(loc, (_, off))| (loc, *off))
            .collect();

        // color @ 20, border radius @ 40, border thickness @ 56. Every vec4 in
        // the layout, and the sprite pipeline -- which renders correctly on the
        // same device -- has none.
        assert_eq!(misaligned, vec![(2, 20), (4, 40), (5, 56)]);
    }
}
